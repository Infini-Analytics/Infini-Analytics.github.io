...
为帮助企业用户应对数据时代新的挑战， ZILLIZ 公司设计研发了新一代海量数据可视化分析引擎 Infini Analytics 。 Infini Analytics 技术方案具有完整的自主知识产权，并率先使用图形处理单元（ GPU ）的大规模并行处理和视觉渲染能力进行数据可视化呈现。与传统方案相比， Infini Analytics 平台具备高吞吐、高性价比、低延时三重优势，单位算力成本降至传统方案的 10% 。用户可以对十亿级数据集进行查询和可视化交互，并获得秒内响应。

![InfiniAnalytics](./image/InfiniAnalytics.png)
...
前端组件基于 React 构建，各类图表基于 D3 开发，用户可以灵活配置各个图表，支持选择多列数据列，进行聚合或者过滤，通过我们特有的 crossfilter sql 生成功能，满足用户进行多 chart 交互操作，并实时更新图表。
...
- 汇总分析纽约出租车一个月的订单情况（超过50万笔）
  ...
  ![taxi01](./image/taxi01.jpg)
  > - 地图定位按订单到达位置显示
  > - 数据点按订单金额着色，越红代表订单金额越高

- 缩放地图，分析指定区域的订单情况
  
  ![taxi04](./image/taxi04.jpg)
  
- 按时间窗口与车辆运营商进行分析过滤
  ![taxi03](./image/taxi03.jpg)
